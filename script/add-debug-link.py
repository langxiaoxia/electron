...
def add_debug_link_into_binary(binary_path, target_cpu, debug_dir):
  if PLATFORM == 'linux' and (target_cpu == 'x86' or target_cpu == 'arm'):
  ...
  if PLATFORM == 'linux' and target_cpu == 'arm64':
    cmd = ['aarch64-linux-gnu-objcopy', '--add-gnu-debuglink=' + debug_name, real_binary_path]
  else:
